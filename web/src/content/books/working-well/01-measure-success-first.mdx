...
import CodeSteps from "../../../components/CodeSteps.astro";
...
The same loop, as code. Only the exit condition changes, and then how the loop ends when it cannot pass.

<CodeSteps
  title="What stops the loop"
  lang="python"
  steps={[
    {
      desc: "The loop stops when the agent thinks it is done.",
      note: "Nothing says stop except the agent's own judgment, so the loop optimises for looking done.",
      code: `def run(task):
    result = agent(task)
    if looks_done(result):
        return result
    return run(task)`,
    },
    {
      desc: "A check decides, and it was written before the loop started.",
      note: "The exit is now a function the author wrote first. The agent cannot talk its way past it.",
      code: `def run(task, check):
    result = agent(task)
    if check(result).passed:
        return result
    return run(task, check)`,
    },
    {
      desc: "The loop gives up, loudly, with the check's report.",
      note: "A bounded loop that fails with evidence beats an unbounded one that eventually passes by accident.",
      code: `def run(task, check, attempts=3):
    result = agent(task)
    if check(result).passed:
        return result
    if attempts == 0:
        raise GaveUp(check(result).report)
    return run(task, check, attempts - 1)`,
    },
  ]}
/>
...
The June review of our own practice[^review] put it this way: write the check before the loop. A loop without a checkable target is just an automated way to make errors you find too late. Late here means at review, or in production, where the cost of a defect is highest and the context that produced it is gone.
...
<Video id="jR670K7R0cs" title="Software Architecture Explained: SOLID Principles & MVC Pattern" />
...
<BookOnly>
<Details title="New to the three layers of verification?">
The agent's own check, automated gates, and human review. Book 1 chapter 5 covers them. This chapter assumes them and argues about where the first layer goes: before the work, not after.
</Details>
</BookOnly>

[^review]: Syv's internal review of its own agentic practice, June 2026.
