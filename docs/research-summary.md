...
27. **[Software Fundamentals Matter More Than Ever](https://www.youtube.com/watch?v=v4F1gFy-hqg)** (Matt Pocock, 2026): Conference talk plus accompanying aihero.dev writing arguing that AI handles the tactical work while design, architecture, quality, naming, and contracts stay human — "bad code is the most expensive it's ever been," because a codebase that's hard to change is one the agent can't work in effectively. Source for several techniques in the playbook: the alignment interview (grill-me), smart-zone vs. dumb-zone context limits, slim instruction files, deterministic hooks as guardrails, and the build-then-rebuild stance. Note the explicit senior-engineer audience — separate the transferable principle from the senior-dev framing and the model-specific numbers before applying it to non-developers.
