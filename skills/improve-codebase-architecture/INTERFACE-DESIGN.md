...
Before starting subagents, write a user-facing explanation of the problem space for the chosen candidate:
...
Start 3 or more subagents in parallel if your harness has them; otherwise draft each design yourself, one constraint at a time, before comparing. Each produces a **radically different** interface for the deepened module.
...
Include the [LANGUAGE.md](LANGUAGE.md) vocabulary in the brief — and, if the project keeps a domain glossary / CONTEXT.md, its vocabulary too — so each subagent names things consistently with the architecture language and the project's domain language.
