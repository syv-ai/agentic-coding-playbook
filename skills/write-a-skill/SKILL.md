...
- **Work with the harness, not around it.** When a skill changes how the user's agent behaves (permissions, safety, automation), configure the harness's own features. Don't install custom hook scripts, wrappers or shims that break silently when the harness or platform changes. If the harness can't do it, say so.
